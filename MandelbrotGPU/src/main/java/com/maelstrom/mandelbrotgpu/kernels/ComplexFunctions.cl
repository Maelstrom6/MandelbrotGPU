...
struct Complex sinhComplex(struct Complex c){
    double x = getRe(c);
    double y = getIm(c);
    return newComplex(sinh(x) * cos(y), cosh(x) * sin(y), false);
}

struct Complex coshComplex(struct Complex c){
    double x = getRe(c);
    double y = getIm(c);
    return newComplex(cosh(x) * cos(y), sinh(x) * sin(y), false);
}
