...
public class Mand {
                    ...
                    for (int x = id; x < sizeX; x += threads) {
                        for (int i = 0; i < sizeY; i++) {
                            ...
                            if (dist == 0) {//need to change this if yuou change i<100
                                ...
                                //double dist = Math.pow(fn.getRe()-f0.getRe(), 2)+Math.pow(fn.getIm()-f0.getIm(), 2);
                                scaler = getRedFromN(dist);//divide everything by 4 for smoother transition
                                scaleg = getGreenFromN(dist);
                                scaleb = getBlueFromN(dist);
    ...
    public double PartOfOrbitDot(double x, double y){
        Myi trapPoint = new Myi(-0.1, 0, false);
        ...
        while (i < maxI && znMinOne.getR() < 2) {
            ...
            i++;
            double zMinusPointModulus = zn.add(trapPoint.ainverse()).getR();
    ...
    public double PartOfOrbitRing(double x, double y){
        double minAcceptable=0.4, maxAcceptable=0.5;
        Myi trapPoint = new Myi(-0.1, 0, false);
        Myi c = new Myi(x, y, false);
        Myi znMinOne = new Myi(x, y, false);
        Myi zn;
    ...
    public double PartOfOrbitCross(double x, double y){
        Myi trapPoint = new Myi(-0.1, 0, false);
        Myi c = new Myi(x, y, false);
        Myi zn = new Myi(x, y, false);
        double distance = 1000000;
        int i = 0;
        while (i < maxI && zn.getR() < 2) {
            zn = zn.power(numer, denom).add(c);
            i++;
            double horiDist = Math.abs(trapPoint.getRe() - zn.getRe());
            double vertDist = Math.abs(trapPoint.getIm() - zn.getIm());
